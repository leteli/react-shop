...
const validate = (entries: Array<[string, string]>): ValidatedForm => {
  ...
  for (const [name, rawValue] of entries) {
    ...
    if (typeof rawValue === 'string') {
      ...
      if (value === '') {
        ...
        validatedData[name] = { isValid: false, error: 'Не более 1000 символов' };
      } else if (name === 'description') {
        validatedData[name] = { isValid: true, value, error: '' };
      } else if (value.length > 60) {
        validatedData[name] = { isValid: false, error: 'Не более 60 символов' };
      } else {
        validatedData[name] = { isValid: true, value,  error: '' };
      }
    }
...
export const getUpdatedData = (validatedData: ValidForm): IUpdatedData => {
